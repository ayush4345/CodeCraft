...
export default function EditorFooter({handleSubmit,submitting,handleAskAI,asking,askMessage}) {
	const [buttonAccess,setButtonAccess] = useState(false)
	const toggleButtonAccess = () => {
        setButtonAccess(prevState => !prevState);
    };
  return (
	...
	<div className='mx-5 my-[10px] flex justify-between w-full'>
		...
		<div className='ml-auto flex items-center space-x-4'>
			...
			onClick={()=>{
				handleSubmit();
				askMessage!=="Accepted" && askMessage!==null && toggleButtonAccess();
			}}
			>
				{!submitting && "Submit"}
